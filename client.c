...
int main(int argc, char **argv) {
  ...
  */
  ...
  struct fac_server serv1 = {
    .ip = "::1",
    .port = 322,
  };

  struct fac_server serv2 = {
    .ip = "::1",
    .port = 228,
  };

  struct fac_server_list second = {
    .server = serv2,
    .next = NULL
  };

  struct fac_server_list first = {
    .server = serv1,
    .next = &second
  };
  ...
  for (int i = 0; i < servers_num; i++) {
    ...
    server_sockaddr.sin6_port = htons(server->port);
    inet_pton(AF_INET6, server->ip, &server_sockaddr.sin6_addr);
    /*
    ...
    */
    ...
    int sck = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
    if (sck < 0) {
    ...
    if (connect(sck, (struct sockaddr *)&server_sockaddr, sizeof(server_sockaddr)) < 0) {
    ...
    if (pthread_create(&server->thread, NULL, (void*(*)(void*))server_recieve_task, (void*)server) != 0) {
